...
import { Container,Left,Right,Label,LabelNotEvents, Info, Date,Text,Field } from './modal-info.styled'
...
export const ModalInfo:FC<IModalProps> = ({dispatch}) => {
  ...
  return (
    ...
    >
        <Container>
            <Left >
                {!selected?.events.length && <LabelNotEvents>Not events</LabelNotEvents>}
            {selected!.events.map(el=>{
                return <Label active={selectedEvent?.id===el.id} onClick={()=>setSelectedEvent(el)}
                >{el.title}</Label>
             })}
            </Left>
         ...
         <Right>
             ...
             {selectedEvent ? (
               <>
                <Text><Field isInline>Title:</Field> {selectedEvent.title}</Text>
                 <Text><Field isInline>Description:</Field>{selectedEvent.description}</Text>
               </>
             ):<Text>{selected!.events.length && !selectedEvent ? 'Select event':'Not event for select'}</Text>}
